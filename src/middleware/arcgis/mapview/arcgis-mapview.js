  ...
  return store => next => async action => {
    switch (action.type) {
      ...
      case actions.INIT_MAP: {
        ...
        // 初始化地图
        ags.view = await window.GeomapUtils.view.map2d.initMapView(Portal, WebmapID, ags.container);
      ...
      case actions.INIT_SPLITMAP: {
        ...
        await prepare();
        // 初始化分屏对比地图
        agstwo.view = await window.GeomapUtils.view.map2d.initMapView(Portal, SplitWebmap, agstwo.containers);
        // 保持左右侧地图保持同步
